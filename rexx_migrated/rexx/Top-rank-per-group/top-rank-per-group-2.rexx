...
*--------------------------------------------------------------------*/
Parse Arg topn .                 /*ª* get number for top N salaries. */
Select
When topn='' Then              /*ª* no argument                    */
topn=1                       /*ª* assume only  1.                */
When topn='?' Then             /*ª* user wants help                */
Call help
When datatype(topn)<>'NUM' Then /*ª* Argument is not a number      */
Call exit 'Invalid argument ('topn'). Must be a number!'
Otherwise
Nop
End
Parse Value '0 0 0 0' with en dn esal. de. deptl
/*ª*employee name,  ID, salary, dept.*/
...
Exit
/* -------------------------------------------------------------------------
 * REXX C:\Users\PeterJ\rexxetta\rexx\Top-rank-per-group\top-rank-per-group-2.rexx was migrated on 23 Mar 2025 at 10:43:33
 * -------------------------------------------------------------------------
*/ 
