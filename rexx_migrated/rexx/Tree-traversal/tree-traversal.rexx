...
**********************************************************************/
If z<>0 &,                           /*ª* it's a node                */
done.z=0 Then Do                  /*ª* not yet done               */
wl=wl z                            /*ª* add it to the preorder list*/
ll.lvl=ll.lvl z                    /*ª* add it to the level list   */
done.z=1                           /*ª* remember it's done         */
End
Return
...
next=0
Parse arg z
If node.z.0left<>0 Then Do           /*ª* there is a left son        */
If node.z.0left.done=0 Then Do     /*ª* we have not visited it     */
next=node.z.0left                /*ª* so we go there             */
node.z.0left.done=1              /*ª* note we were here          */
lvl=lvl+1                        /*ª* increase the level         */
End
End
If next=0 Then Do                    /*ª* not moved yet              */
If node.z.0rite<>0 Then Do         /*ª* there is a right son       */
If node.z.0rite.done=0 Then Do   /*ª* we have not visited it     */
next=node.z.0rite              /*ª* so we go there             */
node.z.0rite.done=1            /*ª* note we were here          */
lvl=lvl+1                      /*ª* increase the level         */
End
End
End
If next=0 Then Do                    /*ª* not moved yet              */
next=node.z.0father                /*ª* go to the father           */
lvl=lvl-1                          /*ª* decrease the level         */
End
Return next                          /*ª* that's the next node       */
/*ª* or zero if we are done     */
...
node.z.0rite  =0
node.0=z
Return z                        /*ª* number of the node just created */
...
Return
/* -------------------------------------------------------------------------
 * REXX C:\Users\PeterJ\rexxetta\rexx\Tree-traversal\tree-traversal.rexx was migrated on 23 Mar 2025 at 10:43:33
 * -------------------------------------------------------------------------
*/ 
