...
say 'Ns: '  Ns
say 'As: '  As;           say
Ns= space( translate(Ns, , ','));  newV2= words(Ns)  /*ª*elide any superfluous blanks from N's*/
As= space( translate(As, , ','));  newV3= words(As)  /*ª*  "    "       "        "      "  A's*/
...
do j=1  for newV2                              /*ª*process each number for  As  and Ns. */
n.j= word(Ns,j);      N= N * n.j           /*ª*get an  N.j  and calculate product.  */
a.j= word(As,j)                            /*ª* "   "  A.j  from the  As  list.     */
end   /*ª*j*/
newV1.=                                              /*ª* [â†“]  converts congruences â”€â”€â”€â–º sets.*/
do i=1  for newV2;        newV3= a.i;    newV1.i.newV3= a.i;    p= a.i
do N;  p= p + n.i;  newV1.i.p= p             /*ª*build a (array) list of modulo values*/
end   /*ª*N*/
end     /*ª*i*/
/*ª* [â†“]  find common number in the sets.*/
do   x=1  for N;  if newV1.1.x==''  then iterate                       /*ª*locate a number. */
do v=2  to newV2;   if newV1.v.x==''  then iterate x                     /*ª*Is in all sets ? */
end   /*ª*v*/
