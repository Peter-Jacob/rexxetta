...
glist=girls
mlist=''
Do ri=1 By 1 Until glist=''            /*ª* as long as there are girls */
Call dbg 'Round' ri
plist=''                             /*ª* no proposals in this round */
glist.=''
Do gi=1 To words(glist)              /*ª* loop over free girls       */
gg=word(glist,gi)                  /*ª* an unmathed girl           */
b=word(pref.gg,1)                  /*ª* her preferred boy          */
plist=plist gg'-'||b               /*ª* remember this proposal     */
glist.b=glist.b gg                 /*ª* add girl to the boy's list */
Call dbg left(gg,glen) 'proposes to' b  /*ª* tell the user              */
End
Do bi=1 To words(boys)               /*ª* loop over all boys         */
b=word(boys,bi)                    /*ª* one of them                */
If glist.b>'' Then                 /*ª* if he's got proposals      */
Call dbg b 'has these proposals' glist.b /*ª* show them               */
End
Do bi=1 To words(boys)               /*ª* loop over all boys         */
b=word(boys,bi)                    /*ª* one of them                */
bm=pos(b'*',mlist)                 /*ª* has he been matched yet?   */
Select
When words(glist.b)=1 Then Do    /*ª* one girl proposed for him  */
gg=word(glist.b,1)             /*ª* the proposing girl         */
If bm=0 Then Do                /*ª* no, he hasn't              */
Call dbg b 'accepts' gg           /*ª* is accepted                */
Call set_mlist 'A',mlist b||'*'||gg /*ª* add match to mlist  */
Call set_glist 'A',remove(gg,glist) /*ª* remove gg from glist*/
pref.gg=remove(b,pref.gg)    /*ª* remove b from gg's preflist*/
End
Else Do                        /*ª* boy has been matched       */
Parse Var mlist newV1 '*' go ' ' /*ª* to girl go             */
If wordpos(gg,pref.b)<wordpos(go,pref.b) Then Do
/*ª* the proposing girl is preferred to the current one */
/*ª* so we replace go by gg     */
Call set_mlist 'B',repl(mlist,b||'*'||gg,b||'*'||go)
Call dbg b 'releases' go
Call dbg b 'accepts ' gg
Call set_glist 'B',glist go  /*ª* add go to list of girls  */
Call set_glist 'C',remove(gg,glist) /*ª* and remove gg     */
End
pref.gg=remove(b,pref.gg)    /*ª* remove b from gg's preflist*/
End
End
...
pick_1:
If bm>0 Then Do                /*ª* boy has been matched       */
Parse Var mlist newV1 '*' go ' ' /*ª* to girl go             */
...
Return s
/* -------------------------------------------------------------------------
 * REXX C:\Users\PeterJ\rexxetta\rexx\Stable-marriage-problem\stable-marriage-problem.rexx was migrated on 23 Mar 2025 at 10:43:30
 * -------------------------------------------------------------------------
*/ 
