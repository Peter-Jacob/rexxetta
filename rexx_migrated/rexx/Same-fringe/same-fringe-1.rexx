...
wl.t=wl.t z                        /*ª* add it to the preorder list*/
...
next=0
Parse arg z,t
If node.t.z.0left<>0 Then Do         /*ª* there is a left son        */
If node.t.z.0left.done=0 Then Do   /*ª* we have not visited it     */
next=node.t.z.0left              /*ª* so we go there             */
node.t.z.0left.done=1            /*ª* note we were here          */
lvl=lvl+1                        /*ª* increase the level         */
End
End
If next=0 Then Do                    /*ª* not moved yet              */
If node.t.z.0rite<>0 Then Do       /*ª* there is a right son       */
If node.t.z.0rite.done=0 Then Do /*ª* we have not visited it     */
next=node.t.z.0rite            /*ª* so we go there             */
node.t.z.0rite.done=1          /*ª* note we were here          */
lvl=lvl+1                      /*ª* increase the level         */
End
End
End
If next=0 Then Do                    /*ª* not moved yet              */
next=node.t.z.0father              /*ª* go to the father           */
...
Return
/* -------------------------------------------------------------------------
 * REXX C:\Users\PeterJ\rexxetta\rexx\Same-fringe\same-fringe-1.rexx was migrated on 23 Mar 2025 at 10:43:27
 * -------------------------------------------------------------------------
*/ 
