...
bordl=bord(tlist)  /*ª* corners that are on the other triangle's edges */
If bordl<>'' Then
Say 'Corners on the other triangle''s edges:' bordl
wb=words(bordl)                     /*ª* how many of them?             */
Select
When wb=3 Then Do                 /*ª* all three match               */
...
Return
End
When wb=2 Then Do                 /*ª* two of them match             */
Say 'Triangles overlap'
Say '  they have a common edge 'bordl
Say ''
Return
End
When wb=1 Then Do                 /*ª* one of them match             */
Say 'Triangles touch on' bordl  /*ª* other parts may overlap       */
Say '  we analyze further'
End
Otherwise                         /*ª* we know nothing yet           */
Nop
End

trio_result=trio(tlist)             /*ª* any other overlap?            */

Select
When trio_result=0 Then Do        /*ª* none whatsoever               */
If wb=1 Then
Say 'Triangles touch (border case) at' show_p(bordl)
Else
Say 'Triangles don''t overlap'
End
When trio_result>0 Then           /*ª* plain overlapping case        */
Say 'Triangles overlap'
End
Say ''
Return
...
Do i=1 To 3
abc.i=subword(abc,i*2-1,2)   /*ª* corners of ABC */
def.i=subword(def,i*2-1,2)   /*ª* corners of DEF */
...
If ssx(s.i t.j) Then Do       /*ª* intersection of two edges     */
over=1
Leave
End
End
...
If over=0 Then Do                   /*ª* no edge intersection found    */
Do ii=1 To 3 Until over           /*ª* look for first possibility    */
Call o '    '  'abc.'ii'='abc.ii 'def='def
Call o 'ii='ii 'def.'ii'='def.ii 'abc='abc
If in_tri(abc.ii,def) Then Do   /*ª* a corner of ABC is in DEF     */
Say abc.ii 'is within' def
over=1
End
Else If in_tri(def.ii,abc) Then Do  /*ª* a corner of DEF is in ABC */
...
*--------------------------------------------------------------------*/
Parse Arg a,b                       /*ª* 2 points                      */
Parse Var a ax ay
Parse Var b bx by
If ax=bx Then Do                    /*ª* vertical line                 */
gk='*'                            /*ª* special slope                 */
gx=ax                             /*ª* x=ax is  the equation         */
gd='*'                            /*ª* not required                  */
End
Else Do
gk=(by-ay)/(bx-ax)                /*ª* compute slope                 */
gd=ay-gk*ax                       /*ª* compute y-distance            */
gx=''                             /*ª* not required                  */
End
Return gk gd gx
...
Parse Arg k,d,x
Select
When k='*' Then res='x='||x       /*ª* vertical line                 */
When k=0   Then res='y='d         /*ª* horizontal line               */
Otherwise Do                      /*ª* ordinary line                 */
Select
...
Parse Arg dd
Select
When dd=0 Then dd=''            /*ª* omit dd if it's zero          */
When dd<0 Then dd=dd            /*ª* use dd as is (-value)         */
Otherwise      dd='+'dd         /*ª* prepend '+' to positive dd    */
End
Return dd
...
Exit 12
/* -------------------------------------------------------------------------
 * REXX C:\Users\PeterJ\rexxetta\rexx\Determine-if-two-triangles-overlap\determine-if-two-triangles-overlap.rexx was migrated on 23 Mar 2025 at 10:43:10
 * -------------------------------------------------------------------------
*/ 
