...
/*�*──────────────────────────────────────────────────────────────────────────────────────*/
MD5: procedure; parse arg newV2;  numeric digits 20  /*�*insure there's enough decimal digits.*/
a='67452301'x;  b="efcdab89"x;  c='98badcfe'x;  d="10325476"x;   x00='0'x;  x80="80"x
newV3=length(newV2)                                 /*�*length in bytes of the input message.*/
L=newV3*8//512;   if L<448  then plus=448 - L   /*�*is the length  less   than  448 ?    */
...
do j=0  to length(newV4) % 64 - 1             /*�*process the message  (lots of steps).*/
anewV5=a;       bnewV5=b;       cnewV5=c;       dnewV5=d  /*�*save the  original values  for later.*/
chunk=j*64                                /*�*calculate the  size  of the chunks.  */
do k=1  for 16            /*�*process the message in chunks.       */
newV1.k=reverse( substr(newV4, chunk + 1 + 4*(k-1), 4) )   /*�*magic stuff.*/
end   /*�*k*/                                        /*�*────step────*/
...
.Lr:    procedure;  parse arg newV5,newV3;           if newV3==0  then return newV5       /*�*left rotate.*/
