/*ª*REXX program to compute and display   Stirling numbers   of the second kind.          */
...
newV1.=
do j=0  for lim+1
newV1.j.j = 1;  if j==0  then iterate   /*ª*define the right descending diagonal.*/
newV1.0.j = 0;  newV1.j.0 = 0               /*ª*   "    "  zero  values.             */
end   /*ª*j*/
max#.= 0                                         /*ª* [â†“]  calculate values for the grid. */
do   n=0  for lim+1;         np= n + 1
do k=1  for lim;           km= k - 1
newV1.np.k = k * newV1.n.k  +  newV1.n.km        /*ª*calculate a number in the grid.      */
max#.k= max(max#.k, newV1.n.k)           /*ª*find the maximum value for a column. */
...
wgi= max(5, length(lim+1) )                      /*ª*the maximum width of the grid's index*/
say 'â•rowâ•'  center("columns", max(9, max#.a + lim), 'â•')  /*ª*display header of the grid.*/
...
say center(r, wgi)  strip( substr(newV2,2), 'T') /*ª*display a single row of the grid.    */
