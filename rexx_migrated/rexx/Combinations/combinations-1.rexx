/*ª*REXX program displays combination sets for X things taken Y at a time.         */
Parse Arg things size chars .  /*ª* get optional arguments from the command line   */
If things='?' Then Do
...
Exit
End
If things==''|things=="," Then things=5  /*ª* No things specified? Then use default*/
If size==''  |size==","   Then size=3    /*ª* No size   specified? Then use default*/
If chars==''|chars=="," Then             /*ª* No chars  specified? Then Use default*/
chars='123456789abcdefghijklmnopqrstuvwxyz'||,
'ABCDEFGHIJKLMNOPQRSTUVWXYZ'||,
"~!@#chars%^&*()_+`{}|[]\:;<>?,./Â¦++++Â±==ËœÂ·" /*ª*some extended chars             */

show_details=sign(size)                  /*ª* -1: Don't show details               */
size=abs(size)
If things<size Then
...
Exit                                           /*ª* stick a fork in it,  we're all */
/*ª*-------------------------------------------------------------------------------*/
...
Exit 13
/* -------------------------------------------------------------------------
 * REXX C:\Users\PeterJ\rexxetta\rexx\Combinations\combinations-1.rexx was migrated on 23 Mar 2025 at 10:43:08
 * -------------------------------------------------------------------------
*/ 
