...
Parse Arg fn
Parse Var fn ou'.'
maxpn = 10000               /*ª* maximum possibilities to check through */
output = ou'.out.txt'
/*ª* read row/col values into rowpp. and colpp. arrays */
cc = linein(fn)
rows = words(cc)
dd = linein(fn)
cols = words(dd)
...
ar = copies('-',rows*cols)
/*ª* values are -=unknown .=blank @=Color */
/*ª* PREFILL  array */
'erase' output
/*ª**********COL PREFILL ************/
Do col = 1 To cols
...
End
End
If pos('-',ar) = 0 Then Do       /*ª* hurray we have a solution */
/*ª* at this point we need to verify solution */
If CHECKBOARD() Then Iterate nextpos   /*ª* too bad didn't match */
sol = sol + 1
...
PROCESSROW:                           /*ª* rowcol pp in, rowcol pp of ol */
prerow = rowcol
len_item = length(rowcol)
...
of = 1
kcnt = 0
/*ª* reduce the left side with already populated values */
...
Leave
End
/*ª* reduce the right side with already populated values */
ofm = len_item + 1 - of
ol = 1
kcnt = 0
Do While (ol < ofm) & (pp <> '')
...
End
End
/*ª* oh oh need to check for posibilities */
Call TRY '',pp
If pn > maxpn Then Do
over = over + 1
Return 0
...
Return len
/* -------------------------------------------------------------------------
 * REXX C:\Users\PeterJ\rexxetta\rexx\Nonogram-solver\nonogram-solver.rexx was migrated on 23 Mar 2025 at 10:43:22
 * -------------------------------------------------------------------------
*/ 
