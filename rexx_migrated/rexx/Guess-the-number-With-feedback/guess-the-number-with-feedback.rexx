/*ª*REXX program plays guess the number  game with a human;  the computer picks the number*/
low=   1                                       /*ª*the lower range for the guessing game*/
high= 100                                       /*ª* "  upper   "    "   "      "      " */
try=   0                                       /*ª*the number of valid (guess) attempts.*/
r= random(low, high)                         /*ª*get a random number  (low â”€â”€â”€â–º high).*/
...
prompt= centre(newV1   low     'and'     high     '(inclusive)  â”€â”€â”€orâ”€â”€â”€  Quit:', 79, "â”€")
/*ª* [â†“]  BY 0 â”€â”€â”€ used to  LEAVE  a loop*/
do ask=0  by 0;     say;      say prompt;      say;     pull g;     g= space(g);    say
do validate=0  by 0                          /*ª*DO index required; LEAVE instruction.*/
select
when g==''                 then iterate ask
when abbrev('QUIT', g, 1)  then exit                              /*ª*what a whoos.*/
